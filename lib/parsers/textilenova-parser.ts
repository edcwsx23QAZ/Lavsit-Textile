...
export class TextileNovaParser extends BaseParser {
  /**
   * Парсинг на Vercel (основная логика)
   */
  ...
  /**
   * Анализ на Vercel (основная логика)
   */
  ...
  /**
   * Публичный метод analyze с fallback
   */
  async analyze(url: string): Promise<ParsingAnalysis> {
    return await this.withFallback(
      // Операция на Vercel
      async () => {
        return await this.analyzeOnVercel(url)
      },
      // Fallback операция
      async () => {
        return await this.callLocalParser('/analyze', { url })
      }
    )
  }
}
