...
export async function POST(
  ...
  try {
    ...
    // Специальная обработка для Аметиста - использует AmetistParser, который сам получает письма
    if (supplier.name === 'Аметист' && supplier.parsingMethod === 'email') {
      console.log(`[parse] Аметист: Using AmetistParser (self-contained email fetching)`)
                  ...
                  for (const email of sortedEmails) {
                    ...
                    if (attachments.length > 0) {
                      ...
                      if (supplier.name === 'Аметист') {
                        ...
                        console.log(`[parse] ${supplier.name}: File validation result: ${isValid}`)
                      }
                ...
                for (const email of sortedEmails) {
                  ...
                  if (attachments.length > 0) {
                    ...
                    if (supplier.name === 'Аметист') {
                      ...
                      const validator = new EmailExcelParser(supplier.id, supplier.name)
                      isValid = await validator.validateFile(tempFilePath)
                      console.log(`[parse] [auto-check-email] ${supplier.name}: File validation result: ${isValid}`)
                    }
